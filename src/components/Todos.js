...
class Todos extends Component {
  constructor(props) {
    ...
    this.handleInputFilterTagChange = this.handleInputFilterTagChange.bind(
      this
    );
  ...
  handleInputFilterTagChange(event) {
    ...
    this.setState({ filterTodoTagTextValue }, () => {
      this.filterTodosFun(filterTodoTagTextValue);
    });
  }
  ...
  filterTodosFun(filterTodoTagTextValue) {
    ...
    if (filterTodoTagTextValue.length === 0) {
      for (var i = 0; i < allTodos.length; ++i) allTodos[i].show = true;
  ...
  addTodo() {
    ...
    if (addTodoDescriptionValue.length) {
      ...
      localStorage.setItem(
        "allTodos",
        JSON.stringify(this.context.store.getState().todos.allTodos)
      );
  ...
  handleTabsView(tabName) {
    ...
    else this.setState({ viewAllTodosTab: false, viewDoneTodosTab: true });
  }
  ...
  render() {
    ...
    return (
      <div>
        <Link to='/'>
          <p className='rightPosition' id='logoutTextLink'>
            {" "}
            Logout
          </p>
        </Link>
          ...
          <Grid item xs={12}>
            ...
            >
                  ...
                  {this.state.viewAllTodosTab ? (
                    <div>
                      <button
                        id='navButtonFocus'
                        className='navButton'
                        onClick={param => this.handleTabsView("all")}
                      >
                        All Todos
                      </button>
                      <button
                        onClick={param => this.handleTabsView("done")}
                        className='navButton'
                      >
                        {" "}
                        Done Todos
              ...
              <Grid item xs={8}>
                <div>
                  <input
                    id='filterByTagInputField'
                    onChange={this.handleInputFilterTagChange}
                    value={this.state.filterTodoTagTextValue}
                    type='text'
                    placeholder='Filter todos by tag..'
                  />
                </div>
              </Grid>
            </Grid>
                      ...
                      <div id='addNewTodoDiv'>
                        ...
                        <Grid item xs={12}>
                          <input
                            ...
                            value={this.state.addTodoTagValue}
                          />
                        </Grid>
                        <Grid item xs={12}>
                          <label className='grayColor1 leftPosition'>
                            {" "}
                            Due Date{" "}
                          </label>
                          <input
                            ...
                            type='datetime-local'
                            name='dueDate'
                            value={String(this.state.addTodoDueDate)}
                            onChange={this.handleDueDateChange}
                            min={Date.now()}
                          />
                        </Grid>
                        <button
                          className='roundedButton todoButtons'
                          onClick={() => {
                            this.handleColorPickerModal(true);
                          }}
                          type='button'
                        >
                          {" "}
                        ...
                        {this.state.openColorPickerModal ? (
                          <SimpleDialog
                            openDialog={this.state.openColorPickerModal}
                            handleDialogClose={this.handleColorPickerModal}
                            dialogContent={
                              <CompactPicker
                                color={this.state.addTodoColor}
